...
if __name__ == "__main__":
    ...
    for move in moves_list:
        ...
        df_moveX['DIFF4CLAS'].mask(( df_moveX['OF_Diff'].apply(lambda x: float(x)) > 0 ) & ( df_moveX['OF_Diff'].apply(lambda x: float(x)) <= 1500 ) ,'Good', inplace=True)
        df_moveX['DIFF4CLAS'].mask( df_moveX['OF_Diff'].apply(lambda x: float(x)) > 1500 ,'Very Good', inplace=True)
        df_move = df_moveX.drop(['OF_Diff'], axis=1)

        df_move.to_csv("./DB-Output-" + move + "-Bal1.csv", sep=',', index=False)

        write_db_bal(df_move["DIFF4CLAS"].value_counts()["Very Good"], df_move["DIFF4CLAS"].value_counts()["Good"], df_move["DIFF4CLAS"].value_counts()["Bad"], move)
