...
# Balancing samples
def write_db_bal(vg_limit, g_limit, n_limit, b_limit, move):
    vg_limit3 = vg_limit * 3
    print("vg_limit: " + str(vg_limit))
    print("g_limit: " + str(g_limit))
    print("n_limit: " + str(n_limit))
    print("b_limit: " + str(b_limit))
    vg_count, g_count, n_count, b_count = 0,0,0,0
    ...
    with open("./DB-Output-" + move + "-Bal1.csv", 'r') as olddb:
        ...
        for row in readerolddb:
            if (row[-1] == "Very Good" and vg_count <= vg_limit3):
                writernewdb.writerow(row)
                writernewdb.writerow(row)
                writernewdb.writerow(row)
                vg_count += 1
            elif (row[-1] == "Good" and g_count <= vg_limit3):
                writernewdb.writerow(row)
                g_count += 1
            elif (row[-1] == "Normal" and n_count <= vg_limit3):
                writernewdb.writerow(row)
                n_count += 1
            elif (row[-1] == "Bad" and b_count <= vg_limit3):
                writernewdb.writerow(row)
                b_count += 1
...
if __name__ == "__main__":
    ...
    for move in moves_list:
        ...
        df_moveX['DIFF4CLAS'].mask( df_moveX['OF_Diff'].apply(lambda x: float(x)) <= 0 ,'Bad', inplace=True)
        df_moveX['DIFF4CLAS'].mask(( df_moveX['OF_Diff'].apply(lambda x: float(x)) > 0 ) & ( df_moveX['OF_Diff'].apply(lambda x: float(x)) <= 1500 ) ,'Normal', inplace=True)
        df_moveX['DIFF4CLAS'].mask(( df_moveX['OF_Diff'].apply(lambda x: float(x)) > 1500 ) & ( df_moveX['OF_Diff'].apply(lambda x: float(x)) <= 3000 ) ,'Good', inplace=True)
        df_moveX['DIFF4CLAS'].mask( df_moveX['OF_Diff'].apply(lambda x: float(x)) > 3000 ,'Very Good', inplace=True)
        df_move = df_moveX.drop(['OF_Diff'], axis=1)

        df_move.to_csv("./DB-Output-" + move + "-Bal1.csv", sep=',', index=False)

        write_db_bal(df_move["DIFF4CLAS"].value_counts()["Very Good"],
                     df_move["DIFF4CLAS"].value_counts()["Good"],
                     df_move["DIFF4CLAS"].value_counts()["Normal"],
                     df_move["DIFF4CLAS"].value_counts()["Bad"],
                     move)
